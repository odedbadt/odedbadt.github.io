...
function render() {
    ...
    for (var ang = 0; ang < 360; ang += 360/11) {
        for (var m = 1; m > -2; m -=2) {
            var transform = function(p) {
                var v1 = shift([-state.W/2, -state.H/2], p);
                var v2 = [v1[0], v1[1]*m]
                var v3 = rotate(ang,v2);
                return shift([state.W/2, state.H/2], v3);
            }
...
function resize() {
  ...
  $('.canvas').css('left', window.innerWidth/10);
  $('.offscreen_buffer').css('left', window.innerWidth/10);
  $('.pallette').attr('height', window.innerHeight);
  $('.pallette').attr('width', window.innerWidth/10);
